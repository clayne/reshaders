...
texture tBlur3 < pooled = true; > { Width = BUFFER_WIDTH/8; Height = BUFFER_HEIGHT/8; Format = RGB10A2; MipLevels = 2;}
texture tBlur4 < pooled = true; > { Width = BUFFER_WIDTH/8; Height = BUFFER_HEIGHT/8; Format = RGB10A2; MipLevels = 2;}
...
float3 blur(sampler src, float2 uv, float2 pxSize, float2 direction)
{
    ...
    color += tex2D(src, uv).rgb * weights[0]; // Center Sampler
    color += tex2D(src, uv + offsets[1] * offset_factor).rgb * weights[1];
    color += tex2D(src, uv + offsets[2] * offset_factor).rgb * weights[2];
    color += tex2D(src, uv + offsets[3] * offset_factor).rgb * weights[3];
    return color;
...
void PS_Blur1(VS_OUTPUT IN, out float3 c : SV_Target0) { c = blur(sBlur1, IN.uv, rcp(tex2Dsize(sBlur1, 0.0)), float2(1.0, 0.0)); }
void PS_Blur2(VS_OUTPUT IN, out float3 c : SV_Target0) { c = blur(sBlur2, IN.uv, rcp(tex2Dsize(sBlur2, 0.0)), float2(0.0, 1.0)); }
void PS_Blur3(VS_OUTPUT IN, out float3 c : SV_Target0) { c = blur(sBlur3, IN.uv, rcp(tex2Dsize(sBlur3, 0.0)), float2(1.0, 0.0)); }
void PS_Blur4(VS_OUTPUT IN, out float3 c : SV_Target0) { c = blur(sBlur4, IN.uv, rcp(tex2Dsize(sBlur4, 0.0)), float2(0.0, 1.0)); }
