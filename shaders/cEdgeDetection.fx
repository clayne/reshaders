...
{
    return sqrt(dot(X, X) + dot(Y, Y)) / sqrt(3.0);
}

float3 NormalizeColor(float3 Color)
{
    return (_NormalizeInput) ? Color / dot(Color, 1.0) : Color;
}
    ...
    */
    ...
    switch(_Select)
    {
        case 0:
            _ddx = ddx(B1);
            _ddy = ddy(B1);
            OutputColor = Magnitude(_ddx, _ddy);
            break;
        case 1:
            OutputColor = (A1 + C1 + B0 + B2) + (B1 * -4.0);
            OutputColor = length(OutputColor) / sqrt(3.0);
            break;
        case 2:
            _ddx = (-A0 + ((-A1 * 2.0) - A2)) + (C0 + (C1 * 2.0) + C2);
            _ddy = (-A0 + ((-B0 * 2.0) - C0)) + (A2 + (B2 * 2.0) + C2);
            OutputColor = Magnitude(_ddx, _ddy);
            break;
        case 3:
        ...
        case 6:
            ...
            OutputColor = Magnitude(Cross, -Cross) / 12.0;
            break;
        case 7:
            _ddx += A0 * -17.0;
            _ddx += A1 * -61.0;
            ...
            break;
        default:
            OutputColor = tex2D(_SampleColor, TexCoord[1].xz).rgb;
            break;
    }
