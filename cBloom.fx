...
texture2D t_Downscaled { Width = 512; Height = 512; MipLevels = 6.0; Format = RGB10A2; };
...
{
	c0 = pCubic(s_Downscaled, input.uv, 1.0);
	c1 = pCubic(s_Downscaled, input.uv, 3.0);
	c2 = pCubic(s_Downscaled, input.uv, 5.0);
}
...
{
	const float2 c0_pt = 1.0 / tex2Dsize(s_C0, 1.0);
	const float2 c1_pt = 1.0 / tex2Dsize(s_C1, 3.0);
			  ...
			  out float3 c2 : SV_Target2)
{
	...
	{
		c0 += tex2Dlod(s_C0, float4(input.c0_uv[i].xy, 0.0, 1.0)).rgb * weights[i];
		c0 += tex2Dlod(s_C0, float4(input.c0_uv[i].zw, 0.0, 1.0)).rgb * weights[i];
		c1 += tex2Dlod(s_C1, float4(input.c0_uv[i+3].xy, 0.0, 3.0)).rgb * weights[i];
		c1 += tex2Dlod(s_C1, float4(input.c0_uv[i+3].zw, 0.0, 3.0)).rgb * weights[i];
...
{
	// Apply motion blur
	const float2 c0_pt = 1.0 / tex2Dsize(s_B0, 1.0);
	const float2 c1_pt = 1.0 / tex2Dsize(s_B1, 3.0);
	...
	[loop] for (int i = 0; i < _Samples; i++)
	{
		c0 += tex2Dlod(s_B0, float4(input.c0_uv[i].xy, 0.0, 1.0)).rgb * weights[i];
		c0 += tex2Dlod(s_B0, float4(input.c0_uv[i].zw, 0.0, 1.0)).rgb * weights[i];
		c0 += tex2Dlod(s_B1, float4(input.c0_uv[i+3].xy, 0.0, 3.0)).rgb * weights[i];
		c0 += tex2Dlod(s_B1, float4(input.c0_uv[i+3].zw, 0.0, 3.0)).rgb * weights[i];
